...
numofocc :: Int [Int] -> Int
numofocc num [] = 0
numofocc num [x:xs] 
| num == x = 1 +  (numofocc num xs)
| otherwise = (numofocc num xs)
//Start = numofocc 1 [1,1,1]
removeOcc :: {Int} -> {Int}
removeOcc arr = {x\\ x<-: arr | isOdd(numofocc x list)}
where 
	list = [x\\ x<-: arr]
...
generatelist 0 = []
generatelist num = generatelist (num-1) ++ repeatn num num 
//Start = generatelist 4
...
calctotalprices :: Shop -> Real 
calctotalprices sh = sum[x.price\\ x<-: sh.products]
//Start = calctotalprices aldi
cheapestShop :: {Shop} -> (String,City)
cheapestShop arrofshops = hd[(x.shopName,x.location)\\ x<-: arrofshops | lowesttotalprices == calctotalprices x ]
where 
	lowesttotalprices = minList[calctotalprices x\\ x<-: arrofshops]
...
remove vec num 
| isMember num vec = [x\\ x<-vec | x<>num]
...
indexOf vec num 
| isMember num vec = hd [y\\ x<-vec & y<-[0,1..] | x==num]
| otherwise = abort"Element does not exist"
...
swap vec num1 num2 
| isMember num1 vec && isMember num2 vec = [x\\ x<- vec | x<> num1]
