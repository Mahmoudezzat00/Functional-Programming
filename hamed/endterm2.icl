...
...
*/
// Start = toChar ( ((( (toInt 'Z') - 65 + 3) rem 26) + 65) )

shiftLetter :: Int Char -> Char 
shiftLetter shift letter = toChar ( ((( (toInt letter) - 65 + shift) rem 26) + 65) )

shiftCipher:: String Int -> String
shiftCipher string shift = toString (map (shiftLetter shift) [x \\ x <-: string])
...
isReachable :: Circle Point -> Bool
isReachable broadcaster point = (distance broadcaster.center point) < (broadcaster.radius)

check :: Circle {Point} -> Bool
check broadcaster array = and (map (isReachable broadcaster) [x \\ x <-: array])
...
instance == Book
where
    (==) livre1 livre2 = (livre1.title == livre2.title) && (livre1.author == livre2.author) && (livre1.pyear == livre2.pyear) && (livre1.num_of_pages == livre2.num_of_pages)
...
instance < Book
where (<) livre1 livre2 = livre1.pyear < livre2.pyear
...
merge l1 l2 = {x \\ x <- (sort libraryList)}
where 
    libraryList = rmDup (list1 ++ list2)
    list1 = [x \\ x <-: l1]
    list2 = [x \\ x <-: l2]


instance + Library
where (+) l1 l2 = {lib_name = (l1.lib_name +++ l2.lib_name), books = (merge l1.books l2.books)}
...
instance == Library
where (==) l1 l2 = ((length [x \\ x <-: l1.books]) == (length [x \\ x <-: l2.books])) && (and [x == y \\ x <-: l1.books & y <-: l2.books])
...
*/
mergeString :: [Char] [Char] -> [Char]
mergeString [x:xs] [y:ys] = [x] ++ [y] ++ (mergeString xs ys)
mergeString [] list = list
mergeString list [] = list
mergeString [] [] = []

mergedArrays :: [String] [String] -> [String]
mergedArrays [] [] = []
mergedArrays [x:xs] [y:ys] = [toString (mergeString (fromString x) (fromString y))] ++ (mergedArrays xs ys)

instance + {String}
where (+) array1 array2 = {x \\ x <- (mergedArrays [x \\ x <-: array1] [x \\ x <-: array2])}
...
height :: (Tree Int) -> Int 
height Leaf = 0
height (Node x lChild rChild) = 1 + (max (height lChild) (height rChild))

AVL_prop_check :: (Tree Int) -> Bool
AVL_prop_check Leaf = True
AVL_prop_check (Node x lChild rChild) = (abs ((height lChild) - (height rChild)) <= 1) && (AVL_prop_check lChild) && (AVL_prop_check rChild)
...
minSum :: [(Int, Int, Int)] Int -> Int
minSum [] min_sum = min_sum 
minSum [x:xs] min_sum 
| (abs (sumOf x)) < min_sum && (x <> (0,0,0)) = minSum xs (abs (sumOf x))
= minSum xs min_sum

sumOf :: (Int, Int, Int) -> Int
sumOf (x, y, z) = x + y + z

smallestTuple :: {(Int, Int, Int)} -> (Int, Int, Int)
smallestTuple array =  hd [x \\ x <-: array | (sumOf x) == (minSum list 100) ]
where list = [x \\ x <-: array ]

multiply :: (Int, Int, Int) [(Int, Int, Int)] -> [(Int, Int, Int)]
multiply tuple [] = []
multiply (x, y, z) [(a,b,c):xs] = [(x*a, y*b, z*c)] ++ (multiply (x,y,z) xs)

alterArray :: {(Int, Int, Int)} -> {(Int, Int, Int)}
alterArray array = {x \\ x <- (multiply (smallestTuple array) [x \\ x <-: array])}
...
where (<) p1 p2 = p1.yearOfMarriage < p2.yearOfMarriage
...
getLastName :: EngagedPerson -> String
getLastName p1 = toString (lastName (fromString p1.fullName))

getOldestMarriage :: {(EngagedPerson, EngagedPerson)} -> String
getOldestMarriage array = getLastName (fst (minList list))
where list = [x \\ x <-: array]
...
getIndex color [] ind = ind 
getIndex color [x:xs] ind 
| x == color = ind 
= getIndex color xs (ind + 1)

match :: Color -> Color
match color = list!!((ind + 2) rem 7)
where 
    ind = getIndex color list 0
    list = [Red, Orange, Yellow, Green, Blue, Indigo, Violet]

neighbours :: [Color] -> [Color]
neighbours list = map match list 
...
depth :: BinaryTree2 -> Int 
depth BTLeaf2 = 1
depth (BTNode2 child1 child2) = 1 + (max (depth child1)(depth child2))

diameter :: BinaryTree2 -> Int 
diameter (BTNode2 child1 child2) = (depth child1) + 1 + (depth child2)

getBTDiameter :: BinaryTree2 -> Int
getBTDiameter BTLeaf2 = 1
getBTDiameter (BTNode2 child1 child2) = maxList [(diameter tree), (getBTDiameter child1), (getBTDiameter child2)]
where tree = (BTNode2 child1 child2)
