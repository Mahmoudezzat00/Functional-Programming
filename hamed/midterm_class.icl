...
splitDigit x
| x < 10 = [x]
= [x rem 10 : splitDigit (x/10)]

isDisariumNum :: Int -> Bool
isDisariumNum n = (sum [x^i \\ x <- reverse (splitDigit n) & i <- [1..(length (splitDigit n))]]) == n
...
*/
harshad :: Int -> Bool
harshad x = ( x rem ( sum (splitDigit x))) == 0
...
*/
max_e :: [Int] -> Int
max_e list = last (sort list)

leaders :: [Int] -> [Int]
leaders [] = []
leaders [x] = [x]
leaders [x:xs]
| x > max_e xs = [x] ++ leaders xs
= leaders xs
...
filteredRem n list = [x rem n \\ x <- list | x rem n <> 0 ]
...
validTriangle (x, y, z) = (x+y)>z && (x+z)>y && (z+y)>x && x>0 && y>0 && z>0

validTriangles :: [(Int,Int,Int)] -> [Bool]
validTriangles list = map validTriangle list
...
*/
strcp :: (String, Int) -> [String]
strcp (s,n) = repeatn n s

stringCopy :: [(String,Int)] -> [[String]]
stringCopy list = map strcp list
...
toDigit :: [Int] -> Int
toDigit [] = 0
toDigit [x:xs] = x*(10^l) + toDigit xs
where l = (length xs)
...
intInsertion x y = toDigit ( comb (reverse (splitDigit x)) (reverse (splitDigit y)) )
...
fail :: [(String, Int)] Int -> [(String,Int)]
fail [] mark = []
fail [x:xs] mark 
| (snd x) < mark = [(fst x, snd x)] ++ fail xs mark
= fail xs mark

pass :: [(String, Int)] Int -> [(String,Int)]
pass [] mark = []
pass [x:xs] mark 
| (snd x) > mark = [(fst x, snd x)] ++ pass xs mark
= pass xs mark

group_by_marks :: [(String, Int)] Int -> ([(String,Int)], [(String,Int)])
group_by_marks list mark = (fail list mark, pass list mark)
...
//Start = toChar(toInt('a') + 3)

isVowel :: Char -> Bool
isVowel c = isMember c ['a', 'e', 'i', 'o', 'u']

countVow :: [Char] -> Int
countVow list = length( filter isVowel list )


cipherList :: [Char] -> [Char]
cipherList list = map (\x = toChar(toInt(x) + (countVow list))) list
