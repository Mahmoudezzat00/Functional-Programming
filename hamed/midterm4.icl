...
todigit num 
| num<9 = [num]
| otherwise = todigit (num/10) ++ [num rem 10]
//Start = todigit 157

isDisariumNum :: Int -> Bool
isDisariumNum num = sum[x^y \\ x<- todigit(num) & y<-[1..]] == num
...
*/
isHarshad :: Int -> Bool
isHarshad num = num rem (sum[x\\ x<- todigit(num)]) == 0

harshadNums :: [Int] -> [Int]
harshadNums list = [x \\ x<- list | isHarshad x]
...
isleader [x:xs]
| x >  xs = [x] ++ (isleader xs)
| otherwise = isleader xs
*/
leaders :: [Int] -> [Int]
leaders [] = []
leaders [n] = [n]
leaders [x:xs]
| x > maxList xs = [x : leaders xs]
...
validTriangles [] = []
validTriangles [(x,y,z):xs]
| x + y > z && x + z > y && y + z > x = [True] ++ validTriangles xs
| otherwise = [False] ++ validTriangles xs 
...
stringCopy [] = []
stringCopy [(str, n):xs]
| n > 0 = [replicate n str : stringCopy xs ]
= [[]:stringCopy xs ]
...
helper :: [Int] [Int] -> [Int]
helper [] [] = []
helper n1 n2 = take 1 n1 ++ take 1 n2 ++ helper(tl n1) (tl n2)
...
intInsertion num1 num2 =  listToNumber (helper (todigit num1) (todigit num2))
...
group_by_marks [(str, grade):xs] passgrade 
| grade > passgrade = ([(str, grade)] ++ group_by_marks xs passgrade )
| grade < passgrade = ([(str, grade)] ++ group_by_marks xs passgrade )
*/
groupStudents :: [(String, Int)] Int -> ([(String, Int)], [(String, Int)])
groupStudents students passMarks =
  partitionStudents students ([], [])
  where
    partitionStudents [] (failed, passed) = (reverse failed, reverse passed)
    partitionStudents [(name, marks):rest] (failed, passed)
      | marks < passMarks = partitionStudents rest ([(name, marks):failed], passed)
      | otherwise = partitionStudents rest (failed, [(name, marks):passed] )

//Start = groupStudents [("Ramesh",23), ("Vivek",40), ("Harsh",88), ("Mohammad",60)] 30
...
reachable (x1, y1) (x2, y2) =
  checkReachable (x1, y1) (x2, y2)
  where
    checkReachable (x, y) (destX, destY)
      | x == destX && y == destY = True
      | x > destX || y > destY = False
      | otherwise = checkReachable (x, x + y) (destX, destY) || checkReachable (x + y, y) (destX, destY)

*/

isReachable :: (Int,Int) (Int,Int) -> Bool
isReachable (x,y) (destX,destY) 
| x == destX && y == destY = True
| x > destX || y > destY = False
| otherwise = isReachable (x, x + y) (destX, destY) || isReachable (x + y, y) (destX, destY)
...
evaluate list num = sum [x*(num^y) \\x <- list & y <- [0..]]
