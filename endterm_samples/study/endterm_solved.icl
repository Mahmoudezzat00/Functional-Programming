...
condVowel str = {a\\ a<-: str | not (isMember a vowelList)}

//condVowel "" = ""
//condVowel x = {a \\ a<-:x | not(isMember a vowelList)}

removeVowels :: {{String}} -> {{String}}
removeVowels arr = {{condVowel b\\b<-: x}\\ x<-:arr}
...
FindLinesT st1 st2 transport = [x.number\\ x<- Linelist | isMember st1 x.stops && isMember st2 x.stops && x.transport == transport]
...
intersection x y = removeDup [a \\ a<- x , b<-y | a==b]
//intersection x y = [a \\ a<-x & a<-y | (isMember a x) && (isMember a y)]

//Start = intersection [1,3,9] [1,2,3,4,1,5] // [1,3]
