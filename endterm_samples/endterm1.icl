...
getBestBScStudent studentlist 
| isEmpty studentlist = abort"Error"
= hd[x.id \\ x<- studentlist | avg(x.grades) == maximumavg && x.level == BSc]
where maximumavg = maxList [avg(x.grades)\\ x <- studentlist | x.level == BSc]
...
//Start = stringtolist "thequickbrownfoxjumpsoverthelazydog"

countchar :: Char [Char] -> Int
countchar chr [] = 0
countchar chr [x:xs] 
| chr == x = 1 + countchar chr xs
= countchar chr xs

//Start = countchar 't' ( stringtolist "thequickbrownfoxjumpsoverthelazydog" )

/*count :: String -> {(Char, Int)}
count str = {(x,b)\\ x<-:listchar & b<- [(countchar (x) listchar )] | b >1 }
where listchar = stringtolist str
*/
...
instance * [Int]
where 
	(*) list1 list2 = [x*y\\ x<- list1 & y<- list2]
	(*) [] [] = []
	(*) [] list2 = []
	(*) list1 [] = []
...
instance - [Int]
where 
	(-) list1 list2 = [x\\ x<- list1 | not (isMember x list2)]
	(-) [] [] = []
	(-) [] list2 = []
	(-) list1 [] = list1
...
firstAndMiddle leaf = []
firstAndMiddle (Node (FirstName x) left right) = [x] ++  firstAndMiddle left ++ firstAndMiddle right
firstAndMiddle (Node (MiddleName x) left right) = [x] ++  firstAndMiddle left ++ firstAndMiddle right
firstAndMiddle (Node (LastName x) left right) =  firstAndMiddle left ++ firstAndMiddle right
...
where 
	(toInt) (A string) = length (stringtolist string)
	(toInt) (B chara) = 1
