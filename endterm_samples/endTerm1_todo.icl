...
where 
	toInt x = length(stringtolist(x.firstName)) + x.age + x.height
...
counti :: String -> Int 
counti str = length [x\\ x<-: str | x =='i' || x == 'I']
//Start = counti "Budapest"
 
isPrime :: Int -> Bool
isPrime 0 = False
isPrime 1 = False
isPrime n = [x\\ x<- [2..(n-1)] | n rem x ==0] == []

//Start = isPrime 15

filtercountry :: Country -> Bool
filtercountry ctry 
| isPrime(numofi) = True
|otherwise = False
where 
	numofi = counti(ctry.capital)
	
//Start = filtercountry Hungary

filtercon :: Continent -> [Country]
filtercon con =  [x \\ x<-: con.countries | filtercountry(x) ]

continentsPrimeI :: [Continent] -> [String]
continentsPrimeI listofcon = [x.contName\\ x<- listofcon | (length (filtercon x)) >= 1  ]
...
::APlayer = { name1 ::String, skillLevel :: Int}

shouldWePlay :: [APlayer] APlayer -> Bool
shouldWePlay listofplayers goalkeeper = length [x.name1\\ x<- listofplayers , y<- [goalkeeper] | x.skillLevel >= y.skillLevel ] >= 3
//Start = shouldWePlay [{name1 = "kareem", skillLevel = 4},{name1 = "Tarek", skillLevel = 3},{name1 = "Ali", skillLevel = 3},{name1="Hussien", skillLevel=2},{name1="Ziad", skillLevel=4}] {name1="Gemy", skillLevel=4} // False
//Start = shouldWePlay [{name1 = "kareem", skillLevel = 5},{name1 = "Tarek", skillLevel = 4},{name1 = "Ali", skillLevel = 3},{name1="Hussien", skillLevel=2},{name1="Ziad", skillLevel=4}] {name1="Gemy", skillLevel=4} // True
...
todigits num 
| num <9 = [num]
=  todigits(num/10) ++ [num rem 10] 

//Start = todigits 12123 

full_words :: Int -> {String}
full_words num = {y+++"-" \\ x<- (todigits num) , y<-: numberList | x == y}
